...
final class StorageListingTests: XCTestCase {
    ...
    func testClientTwoPages() async throws {
        ...
        client.listObjectsV2Results.append(.success(.init(
            contents: [first],
        ...
        var listing = try await StorageListing.create(with: client, input: input)
    ...
    func testClientFailureExpected() async throws {
        ...
        client.listObjectsV2Results.append(.failure(ClientError.expectedError))
        do {
            let _ = try await StorageListing.create(with: client, input: .init())
